...
* 小程序前端，即golfoworld-wx模块和renard-wx模块
  * 微信小程序
* 小程序后端，即golfoworld-wx-api模块
  * Spring Boot 2.x
  * Spring MVC
...
按照项目README文档中的“快速启动”一节，开发者可以快速启动小程序项目。
但是小程序端只可以显示数据和图片，而微信登录会失败、微信支付也会失败，
因为appid不是开发者自己的，

这里进一步介绍开发者需要设置的小程序环境。

### 3.0.1 微信登录配置

开发者在微信小程序官网申请以后，可以有app-id和app-secret信息。

1. 在golfoworld-core模块的src/main/resources的application-core.yml资源文件中设置
    ```
    golfoworld
        wx
            app-id: 开发者申请的app-id
...
2. 在golfoworld-wx模块的project.config.json文件中设置
...
4. 建议开发者关闭当前项目或者直接关闭微信开发者工具，重新打开（因为此时golfoworld-wx模块的appid可能未更新）。
   编译运行，尝试微信登录
...
1. 在golfoworld-core-api模块的src/main/resources的application-core.yml资源文件中设置
    ...
    注意
    > * notify-url是微信商户平台向小程序后台服务发送支付结果的地址。
    >    因此这就要求该地址是可访问的。
    > * 目前小程序后台服务的默认request mapping是`/wx/order/pay-notify`（见WxOrderController类的payNotify）,
...
4. golfoworld-wx的api.js设置云服务器的域名。
   编译运行，尝试微信支付。
...
2. 在golfoworld-core-api模块的src/main/resources的application-core.yml资源文件中设置
...
小程序后端服务每一次请求都会检测是否存在HTTP头部域`X-wx-Token`。
...
注意
> golfoworld-wx模块代码基于nideshop-mini-program的commit版本[acbf6276eb27abc6a48887cddd223d7261f0088e](https://github.com/tumobi/nideshop-mini-program/commit/acbf6276eb27abc6a48887cddd223d7261f0088e)。
> 由于改动变化较大，因此之后golfoworld-wx将独立开发，不会合并nideshop-mini-program的更新。
...
golfoworld-wx模块采用storage来存储一些数据，以及支持组件间数据通信。
...
renard-wx是另外一个小程序前端，其后端API也是golfoworld-wx-api。
