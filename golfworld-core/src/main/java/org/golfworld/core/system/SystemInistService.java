...
import org.golfworld.core.util.SystemInfoPrinter;
...
class SystemInistService {
    ...
    static {
        ...
        DEFAULT_CONFIGS.put(SystemConfig.WX_INDEX_CATLOG_LIST, "4");
        DEFAULT_CONFIGS.put(SystemConfig.WX_INDEX_CATLOG_GOODS, "4");
        ...
        DEFAULT_CONFIGS.put(SystemConfig.ORDER_UNCONFIRM, "7");
        DEFAULT_CONFIGS.put(SystemConfig.ORDER_COMMENT, "7");
        // 商城相关配置默认值
        DEFAULT_CONFIGS.put(SystemConfig.MALL_NAME, "litemall");
        ...
        DEFAULT_CONFIGS.put(SystemConfig.MALL_PHONE, "021-xxxx-xxxx");
        DEFAULT_CONFIGS.put(SystemConfig.MALL_QQ, "705144434");
    }

    @Autowired
    private SystemConfigService litemallSystemConfigService;
    ...
    private Map<String, String> getSystemInfo() {
        ...
        infos.put("调试级别", environment.getProperty("logging.level.org.golfworld.wx"));

        // 测试获取application-core.yml配置信息
        infos.put(SystemInfoPrinter.CREATE_PART_COPPER + 1, "模块状态");
        infos.put("邮件", environment.getProperty("application.notify.mail.enable"));
        infos.put("短信", environment.getProperty("application.notify.sms.enable"));
        infos.put("模版消息", environment.getProperty("application.notify.wx.enable"));
        infos.put("快递信息", environment.getProperty("application.express.enable"));
        infos.put("快递鸟ID", environment.getProperty("application.express.appId"));
        infos.put("对象存储", environment.getProperty("application.storage.active"));
        infos.put("本地对象存储路径", environment.getProperty("application.storage.local.storagePath"));
        infos.put("本地对象访问地址", environment.getProperty("application.storage.local.address"));
        infos.put("本地对象访问端口", environment.getProperty("application.storage.local.port"));

        // 微信相关信息
        infos.put(SystemInfoPrinter.CREATE_PART_COPPER + 2, "微信相关");
        infos.put("微信APP KEY", environment.getProperty("application.wx.app-id"));
        infos.put("微信APP-SECRET", environment.getProperty("application.wx.app-secret"));
        infos.put("微信支付MCH-ID", environment.getProperty("application.wx.mch-id"));
        infos.put("微信支付MCH-KEY", environment.getProperty("application.wx.mch-key"));
        infos.put("微信支付通知地址", environment.getProperty("application.wx.notify-url"));
