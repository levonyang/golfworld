...
import org.golfworld.core.express.ExpressService;
import org.golfworld.core.express.dao.ExpressInfo;
...
import org.golfworld.core.util.DateTimeUtil;
...
import org.golfworld.db.util.CouponUserConstant;
import org.golfworld.db.util.GrouponConstant;
import org.golfworld.db.util.OrderHandleOption;
import org.golfworld.db.util.OrderUtil;
import org.golfworld.core.util.IpUtil;
import org.golfworld.wx.task.OrderUnpaidTask;
...
import static org.golfworld.wx.util.WxResponseCode.*;
...
//        }
//
//        List<Short> ordergtatus = OrderUtil.orderStatus(showType);
...
//            }
//
//            List<OrderProduct> orderProductList = orderProductService.queryByOid(o.getId());
//            List<Map<String, Object>> orderProductVoList = new ArrayList<>(orderProductList.size());
//            for (OrderProduct orderProduct : orderProductList) {
//                Map<String, Object> orderProductVo = new HashMap<>();
//                orderProductVo.put("id", orderProduct.getId());
//                orderProductVo.put("productName", orderProduct.getProductName());
//                orderProductVo.put("number", orderProduct.getNumber());
//                orderProductVo.put("picUrl", orderProduct.getPicUrl());
//                orderProductVo.put("specifications", orderProduct.getSpecifications());
//                orderProductVo.put("price",orderProduct.getPrice());
//                orderProductVoList.add(orderProductVo);
//            }
//            orderVo.put("productList", orderProductVoList);
//
...
//        return ResponseUtil.okList(orderVoList, orderList);
    ...
    public Object detail(Integer userId, Integer orderId) {
        ...
        orderVo.put("productPrice", order.getProductPrice());
...
//        List<OrderProduct> orderProductList = orderProductServicevgc.queryByOid(order.getId());

        Map<String, Object> result = new HashMap<>();
        result.put("orderInfo", orderVo);
//        result.put("orderProduct", orderProductList);
...
//            GrouponRules rules = grouponRulesService.findById(grouponRulesId);
//            //找不到记录
//            if (rules == null) {
//                return ResponseUtil.badArgument();
//            }
...
//                }
//                // （2）不允许参加自己开团的团购
//                Groupon groupon = grouponService.queryById(userId, grouponLinkId);
...
//
//        // 收货地址
//        Address checkedAddress = addressService.query(userId, addressId);
...
//        GrouponRules grouponRules = grouponRulesService.findById(grouponRulesId);
//        if (grouponRules != null) {
//            grouponPrice = grouponRules.getDiscount();
//        }
//
//        // 货品价格
//        List<Cart> checkedProductList = null;
//        if (cartId.equals(0)) {
//            checkedProductList = cartService.queryByUidAndChecked(userId);
//        } else {
//            Cart cart = cartService.findById(cartId);
//            checkedProductList = new ArrayList<>(1);
//            checkedProductList.add(cart);
//        }
//        if (checkedProductList.size() == 0) {
//            return ResponseUtil.badArgumentValue();
//        }
//        BigDecimal checkedProductPrice = new BigDecimal(0);
//        for (Cart checkProduct : checkedProductList) {
//            //  只有当团购规格商品ID符合才进行团购优惠
//            if (grouponRules != null && grouponRules.getProductId().equals(checkProduct.getProductId())) {
//                checkedProductPrice = checkedProductPrice.add(checkProduct.getPrice().subtract(grouponPrice).multiply(new BigDecimal(checkProduct.getNumber())));
//            } else {
//                checkedProductPrice = checkedProductPrice.add(checkProduct.getPrice().multiply(new BigDecimal(checkProduct.getNumber())));
//            }
//        }
//
//        // 获取可用的优惠券信息
...
//            Coupon coupon = couponVerifyService.checkCoupon(userId, couponId, userCouponId, checkedProductPrice);
//            if (coupon == null) {
//                return ResponseUtil.badArgumentValue();
//            }
...
//        if (checkedProductPrice.compareTo(SystemConfig.getFreightLimit()) < 0) {
...
//
//        // 订单费用
//        BigDecimal orderTotalPrice = checkedProductPrice.add(freightPrice).subtract(couponPrice).max(new BigDecimal(0));
//        // 最终支付费用
...
//        order.setProductPrice(checkedProductPrice);
...
//
//        // 添加订单商品表项
//        for (Cart cartProduct : checkedProductList) {
//            // 订单商品
//            OrderProduct orderProduct = new OrderProduct();
//            orderProduct.setOrderId(order.getId());
//            orderProduct.setProductId(cartProduct.getProductId());
//            orderProduct.setProductSn(cartProduct.getProductSn());
//            orderProduct.setProductId(cartProduct.getProductId());
//            orderProduct.setProductName(cartProduct.getProductName());
//            orderProduct.setPicUrl(cartProduct.getPicUrl());
//            orderProduct.setPrice(cartProduct.getPrice());
//            orderProduct.setNumber(cartProduct.getNumber());
//            orderProduct.setSpecifications(cartProduct.getSpecifications());
//            orderProduct.setAddTime(LocalDateTime.now());
//
//            orderProductService.add(orderProduct);
//        }
//
//        // 删除购物车里面的商品信息
//        cartService.clearProduct(userId);
//
//        // 商品货品数量减少
//        for (Cart checkProduct : checkedProductList) {
//            Integer productId = checkProduct.getProductId();
//            ProductProduct product = productService.findById(productId);
//
//            int remainNumber = product.getNumber() - checkProduct.getNumber();
//            if (remainNumber < 0) {
//                throw new RuntimeException("下单的商品货品数量大于库存量");
//            }
//            if (productService.reduceStock(productId, checkProduct.getNumber()) == 0) {
...
//        Short comments = orderProductService.getComments(orderId);
//        order.setComments(comments);
     ...
     * @return 待评价订单商品信息
     */
    public Object product(Integer userId, Integer orderId, Integer productId) {
        return null;
//        if (userId == null) {
//            return ResponseUtil.unlogin();
//        }
...
//        }
//
//        List<OrderProduct> orderProductList = orderProductService.findByOidAndGid(orderId, productId);
...
//        return ResponseUtil.ok(orderProduct);
    }
...
//        }
//
//        Integer orderProductId = JacksonUtil.parseInteger(body, "orderProductId");
//        if (orderProductId == null) {
//            return ResponseUtil.badArgument();
//        }
//        OrderProduct orderProduct = orderProductService.findById(orderProductId);
...
//        }
//        Integer commentId = orderProduct.getComment();
//        if (commentId == -1) {
...
//        comment.setValueId(orderProduct.getProductId());
...
//
//        // 2. 更新订单商品的评价列表
//        orderProduct.setComment(comment.getId());
