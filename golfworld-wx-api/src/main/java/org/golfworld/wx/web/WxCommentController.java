...
public class WxCommentController {
    ...
    private Object validate(Comment comment) {
        ...
        if (type == CommonTypeConstant.PRODUCT_COMMENT) {
            if (productService.findById(valueId) == null) {
                return ResponseUtil.badArgumentValue();
            }
        } else if (type == CommonTypeConstant.PRODUCT_TALKING) {
//            if (topicService.findById(valueId) == null) {
//                return ResponseUtil.badArgumentValue();
//            }
        } else if (type == CommonTypeConstant.REPLY) {
            if (comment.getReplyTo() == null) {
                return ResponseUtil.fail(400, "reply to can not be null");
            }
        }
...
//            comment.setPicUrls(new String[0].toString());
    ...
    public Object delete(@LoginUser Integer userId,
        ...
        if (null == comment) {
            return ResponseUtil.fail(404, "comment not exist");
        }
    ...
    public Object list(
        ...
        List<CommentInfo> commentVoList = commentDecorator.convertList(commentList, userId);
